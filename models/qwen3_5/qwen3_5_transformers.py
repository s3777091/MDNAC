...
def _fast_path_unavailable_reason():
    reasons = []
...
class Qwen3_5GatedDeltaNet(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        self.causal_conv1d_update = (
            causal_conv1d_update if self.fast_path_enabled else torch_causal_conv1d_update
        )
        self.chunk_gated_delta_rule = (
            chunk_gated_delta_rule if self.fast_path_enabled else torch_chunk_gated_delta_rule
        )
        self.recurrent_gated_delta_rule = (
            fused_recurrent_gated_delta_rule if self.fast_path_enabled else torch_recurrent_gated_delta_rule
        )

        if not self.fast_path_enabled:
            logger.warning_once(
                "The Qwen3.5 fast path is unavailable ("
                + _fast_path_unavailable_reason()
                + "). Falling back to the torch implementation. "
                "To enable it, install https://github.com/fla-org/flash-linear-attention#installation and "
                "https://github.com/Dao-AILab/causal-conv1d in a CUDA-capable environment."
            )
