...
from libs.data.training.tokenizer.cache_io import (
    _array_typecode_for_vocab_size,
    _encoded_sequence_byte_size,
    _read_token_id_sequence,
    _replace_pair_once_in_sequence,
    _write_token_id_sequence,
)
from libs.data.training.tokenizer.resume import (
    _emit_progress,
    _load_tokenizer_resume_state,
    _stats_from_resume_payload,
    _tokenizer_resume_paths,
    _write_tokenizer_resume_state,
)
from libs.data.training.tokenizer.parallel import (
    _build_token_cache_chunks,
    _count_cached_pairs_parallel,
    _rewrite_token_cache_with_merge_parallel,
)
...
class SequenceTokenizer:
    ...
    def train_from_text_file(
        ...
        resolved_worker_count = _resolve_worker_count(worker_count)
        ...
        try:
            ...
            if resolved_worker_count > 1:
                try:
                    executor = ProcessPoolExecutor(max_workers=resolved_worker_count)
                except OSError as exc:
                    _emit_progress(
                        progress_callback,
                        {
                            "event": "tokenizer_parallel_disabled",
    ...
    def _count_cached_pairs(
        ...
        *,
        ...
        if worker_count > 1 and executor is not None and total_bytes > 0:
            chunks = _build_token_cache_chunks(cache_path, id_typecode=id_typecode, worker_count=worker_count)
            if len(chunks) > 1:
                return _count_cached_pairs_parallel(
                    cache_path,
                    id_typecode=id_typecode,
                    chunks=chunks,
                    merge_index=merge_index,
    ...
    def _rewrite_token_cache_with_merge(
        ...
        *,
        ...
        if worker_count > 1 and executor is not None and total_bytes > 0:
            chunks = _build_token_cache_chunks(source_path, id_typecode=id_typecode, worker_count=worker_count)
            if len(chunks) > 1:
                _rewrite_token_cache_with_merge_parallel(
                    source_path,
                    target_path,
...
def _resolve_worker_count(worker_count: int) -> int:
    if worker_count < 0:
        raise ValueError("worker_count must be greater than or equal to 0.")
    if worker_count == 0:
